...
t <- sample_n(CA_500_plus1.sample,size = 100)
library(splitstackshape)
t<-cSplit(t, 'property_type', sep=":", type.convert=FALSE)
...
## Write jcash_by_zip to file to read into shiny app. 
write_csv(jcash_by_zip, path = "shiny_data/zip_hh_owed")

## Genenerate ggplot showing distribution of zipcodes by 
jcash_by_zip %>% ggplot(aes(x=reorder(owner_zip, desc(value)), y = value, color=income_hh)) + 
  geom_col() +
  labs(x="Zip Code",
       y="Unclaimed Value per Zip") +
